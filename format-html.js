...
let targetDir;
const possiblePaths = [
  path.join(__dirname, 'frontend', 'public', 'downloaded_articles'),
  path.join(__dirname, 'public', 'downloaded_articles'),
  path.join(__dirname, '..', 'public', 'downloaded_articles'),
  path.join(__dirname, 'downloaded_articles')
];
...
// 如果没有找到任何路径，使用当前工作目录
if (!targetDir) {
  targetDir = path.join(process.cwd(), 'frontend', 'public', 'downloaded_articles');
  console.log(`未找到目标目录，尝试使用: ${targetDir}`);
}
